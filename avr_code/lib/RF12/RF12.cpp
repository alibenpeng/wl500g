...
static uint16_t frequency;           // frequency
...
*/
void rf12_initialize (uint8_t id, uint8_t band, uint8_t g, uint16_t f) {
    nodeid = id;
    group = g;
				frequency = f;
    ...
    //rf12_xfer(0xA67C); // 868.3000MHz 
    rf12_xfer(frequency); // 868MHz 
